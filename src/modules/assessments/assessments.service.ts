...
//    * OPTIMIZATION: Single query with randomization at DB level
//    */
//   async startTest() {
//     const startTime = Date.now();

//     // ✅ OPTIMIZED: Get 60 random questions in ONE query
//     // Using PostgreSQL's RANDOM() function is faster than:
//     // 1. Fetching all questions
//     // 2. Shuffling in Node.js
//     // 3. Slicing to 60
...
//         `Not enough active questions. Found ${questions.length}, need 60`,
//       );
//     }
...
//    * OPTIMIZATION: Batch operations and efficient queries
//    */
...
//   /**
//    * Get test result by session token
//    * OPTIMIZATION: Single query with select
//    */
...
//    * OPTIMIZATION: Aggregation at DB level
//    */
//   async getStatistics() {
//     const stats = await this.prisma.$queryRaw<
//       Array<{
//         total_tests: bigint;
//         avg_r: number | null; // ✅ Can be null
//         avg_i: number | null;
//         avg_a: number | null;
//         avg_s: number | null;
//         avg_e: number | null;
...
//     const totalTests = Number(firstRow?.total_tests ?? 0);
...
//    * OPTIMIZATION: Use crypto.randomUUID for better performance
//    */
...
//    * OPTIMIZATION: Batch delete at DB level
//    */
  ...
  export class AssessmentsService {
    ...
    async startTest() {
      ...
      try {
        ...
        this.logger.log(
          `🎉 Session ${session.sessionToken} started in ${Date.now() - startTime}ms`,
        );
...
this.logger.debug('User scores:', scoringResult.scores);
      ...
      this.logger.log(
        `Test ${result.id} completed in ${Date.now() - startTime}ms`,
      );
    ...
    async getStatistics() {
      ...
      const topCodes = await this.prisma.$queryRaw<
        Array<{ career_code: string; count: bigint }>
      >`
        SELECT career_code, COUNT(*) as count
        FROM test_results
        GROUP BY career_code
      ...
      const tierDist = await this.prisma.$queryRaw<
