// import { DEFAULTS, MATCH_THRESHOLDS, MATCH_TYPES, RIASEC_CATEGORIES } from '../../scoring/constants/scoring.constants';
...
// // ✅ ADD: Career database type
// export interface CareerFromDB {
//   id: number;
//   careerName: string;
//   description: string;
//   profile: any; // JSON from Prisma
//   jobZone: number;
//   tags: string[];
// }

// /**
//  * ✅ NEW: Normalize career profile from database
//  * Handles different JSON formats that might come from DB
//  */
...
//     // Validate all RIASEC categories exist
//     for (const cat of RIASEC_CATEGORIES) {
//       if (typeof data[cat] !== 'number') {
//         console.warn(`Invalid profile - missing or invalid ${cat}:`, data);
//         return null;
//       }
//     }
    
//     return data as RIASECScores;
//   } catch (error) {
...
// /**
//  * Categorize correlation strength into match types.
//  */
// export function getMatchType(correlation: number): string | null {
//   if (correlation >= MATCH_THRESHOLDS.BEST_FIT) return MATCH_TYPES.BEST_FIT;
//   if (correlation >= MATCH_THRESHOLDS.GREAT_FIT) return MATCH_TYPES.GREAT_FIT;
//   if (correlation >= MATCH_THRESHOLDS.GOOD_FIT) return MATCH_TYPES.GOOD_FIT;
//   return null;
// }

// /**
//  * ✅ IMPROVED: Match user profile against career database
//  */
...
//     // ✅ NORMALIZE PROFILE from database
//     const careerProfile = normalizeProfile(career.profile);
    
//     if (!careerProfile) {
//       skippedCount++;
//       console.warn(`Skipping career ${career.id} (${career.careerName}) - invalid profile`);
//       return;
//     }
...
//     } catch (error) {
//       skippedCount++;
//       console.error(`Error matching career ${career.id}:`, error);
//     }
//   });
...
// /**
//  * Filter career matches based on user preferences.
//  */
// export function filterCareersByPreferences(
//   matches: CareerMatch[],
...
//       if (!preferences.preferredJobZones.includes(match.jobZone)) return false;
//     }

//     if (preferences.minJobZone && match.jobZone < preferences.minJobZone) {
//       return false;
//     }
//     if (preferences.maxJobZone && match.jobZone > preferences.maxJobZone) {
//       return false;
//     }

//     if (preferences.preferredTags?.length) {
//       const hasPreferredTag = preferences.preferredTags.some((tag) =>
...
//       const hasExcludedTag = preferences.excludeTags.some((tag) =>
//         match.tags.includes(tag),
//       );
...
// /**
//  * Calculate aggregate statistics for a set of career matches.
//  */
// export function getMatchStatistics(matches: CareerMatch[]): MatchStatistics {
...
//     stats.avgCorrelation = Math.round(sum / matches.length * 1000) / 1000; // Round to 3 decimals
//   }
...
// /**
//  * Complete matching workflow: match careers, apply filters, and generate statistics.
//  */
...
//   // Match all careers first
//   let matches = matchCareers(userScores, careers, careers.length);

//   // Apply preference filters
//   if (preferences) {
//     matches = filterCareersByPreferences(matches, preferences);
//   }
...
export function matchCareers(
  ...
  console.log(`Matching ${filteredCareers.length}/${careers.length} careers after preference filtering`);
  ...
  filteredCareers.forEach((career) => {
    ...
    try {
      ...
      skippedCount++;
      errors.push(`Career ${career.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  });
...
/**
 * ✅ OPTIMIZED: Complete workflow with better algorithm
 * Time Complexity: O(n log n) dominated by sorting
 * 
 * KEY IMPROVEMENTS:
 * 1. Preferences filter applied DURING matching (not after)
 * 2. Guaranteed non-empty results
